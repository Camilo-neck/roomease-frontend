...
const CreateTaskModal = ({
	...
	const { register, handleSubmit, watch, control, getFieldState, formState: { errors } } = useForm({ defaultValues: initialState });
	...
	return (
		<Dialog open={isOpen} onClose={handleClose} className='rounded-2xl'>
			...
			<DialogContent>
				<DialogContentText>
					Crea y asigna una tarea a un usuario o a ti mismo.
				</DialogContentText>
				<form ref={formRef} onSubmit={handleSubmit(handleOnSubmit)}>
					...
					<div className='grid grid-cols-2 gap-5'>
						<Controller
							...
							name="start_date"
							render={
								({ field: {ref, onBlur, name, ...field}, fieldState }) => (
					...
					<Controller 
						...
						render={({ field: { onChange, value } }) => (
							<Autocomplete
								...
								value = {
									value ?
									users.filter((user) => value.includes(user._id as never)) ?? []
									: []
								}
								options={users}
