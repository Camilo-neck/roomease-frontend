...
export default function CreateHouseModal({
	...
	const loadImageToFirebase = async (file: File) => {
		...
		if (file) uploadTask.on('state_changed', (snapshot) => {
			...
			getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
				...
				setImage(file.name);
				reset(formValues => ({ ...formValues, house_picture: downloadURL }));
				setImageLoading(false);
			});
		});
	}
    ...
    return (
        <Dialog open={isOpen} onClose={handleClose} className="rounded-2xl">
            ...
            <DialogContent>
                <DialogContentText>
                    Registra tu casa en nuestra app y compártela con otros
                    usuarios. ¡Es fácil y rápido!
                </DialogContentText>
                <form ref={formRef} onSubmit={handleSubmit(handleOnSubmit)}>
                    ...
                    <TextField
                        ...
                        {...register("tags", { required: true, pattern: /([a-zA-Z0-9]+,?\s?)+/  })}
                    />
					<div className='flex flex-row items-center gap-4 mt-2'>
						...
						</label>
						{
							...
							(
								<p className="max-w-32 truncate text-ellipsis">{image}</p>
							)
						}
						{
							image && (
